...
c1 = cur.execute("SELECT country, shortName, name, activityType, ecContribution, organizationURL, COUNT(organizationURL) FROM Participants WHERE role = 'coordinator' AND country = 'acronym_selected' GROUP BY organizationURL ORDER BY ecContribution DESC")
