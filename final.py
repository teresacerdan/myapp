...
participants=pd.read_sql("SELECT country, shortName, name, activityType, SUM(ecContribution), organizationURL, COUNT(organizationURL) FROM Participants WHERE role = 'participant' AND country = 'ES' GROUP BY organizationURL ORDER BY SUM(ecContribution)DESC".format(my_acronym),conn)
